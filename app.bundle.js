...
function adminAddItem(){
  ...
  const is_active = document.getElementById('new_active').checked;

  if (!title){ msg.innerHTML = '<div class="alert">Укажите название</div>'; return; }
  if (!(price > 0)){ msg.innerHTML = '<div class="alert">Укажите корректную цену в баллах</div>'; return; }
  ...
  apiCall({
    ...
    token,
    // В твоём doPost adminAddCatalogItem ждёт payload через JSON? Нет, он ждёт (token, payload) прямо,
    // но ты там делаешь api_adminAddCatalogItem(token, JSON.parse(data.payload));
    // Значит нам надо передавать payload как JSON.stringify({...})
    // Делаем так:
  ...
  .then(() => {
    ...
    if (document.getElementById('new_category_select')) document.getElementById('new_category_select').value='';
    if (document.getElementById('new_category_custom')){
      document.getElementById('new_category_custom').value='';
      document.getElementById('new_category_custom').style.display='none';
    }
